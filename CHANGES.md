# Patch Version Update Truck - 3 - 2

## Bugfixes
- Errors on the home page not generated by the calculator directly are now properly displayed.
- The input type for the calculator collateral and volume boxes has been changed to `number`, which preempts a failed server-side `is_numeric()` check when using thousands separators. 
    - This is an imperfect solution, but better ones are far more complicated and tend to involve third-party libraries. 
- The Homepage and Hauling Dashboard no longer error out if a selected Contract Data Source's Refresh Token has gone invalid.

### UPDATE INSTRUCTIONS (From Version Pickup – 0 – *)

1. Pause operation of the webserver.
2. Sync up files with the repository.
3. Restart operation of the webserver.
